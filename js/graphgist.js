...
function GraphGist($) {
  ...
  var $QUERY_ERROR_LABEL = $(
    '<span class="label label-important query-info">Test run Error</span>');
  var $TOGGLE_BUTTON = $('<span data-toggle="tooltip"><i class="' +
    COLLAPSE_ICON + '"></i></span>');
  var $QUERY_TOGGLE_BUTTON = $TOGGLE_BUTTON.clone().addClass('query-toggle').attr(
    'title', 'Show/hide query.');
  var $RESULT_TOGGLE_BUTTON = $TOGGLE_BUTTON.clone().addClass('result-toggle').attr(
    'title', 'Show/hide result.');
  ...
  var DEFAULT_SOURCE =
    'github-Pema-sDaddy/graphgist/contents/gists/home.adoc'
  ...
  function renderContent(originalContent, link, imagesdir) {
    ...
    try {
      ...
      errorMessage('Error while parsing the AsciiDoc source - ' + e.name + ':' +
        '<p>' + e.message + '</p>');
      return;
    }
    ...
    var consoleUrl = CONSOLE_VERSIONS[version in CONSOLE_VERSIONS ? version :
      DEFAULT_VERSION];
    CypherConsole({
      'url': consoleUrl
    }, function(conslr) {
  ...
  function postProcessRendering() {
    ...
    $('a.run-query,a.edit-query,a.show-console-toggle').tooltip({
      'placement': 'right'
    });
  ...
  function postProcessPage() {
    ...
    if (author) {
      var authorHtml = '<i class=' + (twitter ? '"icon-twitter-sign"' :
        '"icon-user"') + '></i> Author ';
      if (twitter) {
    ...
    $footer.prepend(
      ...
      version + '/cypher-query-lang.html">' + version + '</a> ');
    $('h2[id]').css({
      cursor: 'pointer'
    }).click(function() {
    ...
    $('code', $content).each(function(index, el) {
      ...
      if ($el.hasClass('cypher')) {
        ...
        $toggleQuery.click(function() {
          var $icon = $('i', this);
          var $queryWrapper = $icon.parent().prevAll(
            'div.query-wrapper').first();
          var action = toggler($queryWrapper, this);
          if (action === 'hide') {
            var $queryMessage = $queryWrapper.nextAll(
              'pre.query-message').first();
  ...
  function executeQueries(callbackAfter) {
    ...
    function success(data, resultNo) {
      ...
      createQueryResultButton($QUERY_OK_LABEL, $wrapper, data.result, !
        showOutput);
    ...
    $('#content pre.highlight.setup-query > div.query-wrapper').each(function() {
      ...
      $wrapper.prevAll('h5').first().each(function() {
        ...
        $heading.text($heading.text() +
          ' — this query has been used to initialize the console');
      });
    });
  ...
  function renderGraphs() {
    var counter = 0;
    findPreviousQueryWrapper('h5.graph-visualization', $content, function(
      $heading, $wrapper) {
      var visualization = $wrapper.data('visualization');
      var id = 'graph-visualization-' + (counter++);
      var $visContainer = $VISUALIZATION.clone().attr('id', id).insertAfter(
        $heading);
      var style = $heading.attr('data-style');
      if (style == "{style}") style = null;
      var show_result_only = $heading.attr('graph-mode') && $heading.attr(
        'graph-mode').indexOf('result') !== -1;
      var selectedVisualization = handleSelection(visualization,
        show_result_only);
      $heading.remove();
      ...
      function performVisualizationRendering() {
        if (visualization) {
          var rendererHooks = neod3Renderer.render(id, $visContainer,
            selectedVisualization, style);
          var subscriptions = 'subscriptions' in rendererHooks ?
            rendererHooks['subscriptions'] : {};
          var actions = 'actions' in rendererHooks ? rendererHooks[
            'actions'] : {};
          var $visualizationIcons = $VISUALIZATION_ICONS.clone().appendTo(
            $visContainer);
          $visualizationIcons.children('i.fullscreen-icon').click(
            fullscreenClick);
          for (var iconName in actions) {
            var actionData = actions[iconName];
            $I.clone().addClass(iconName).attr('title', actionData.title).appendTo(
              $visualizationIcons).click(actionData.func);
          }
          $visContainer.mutate('width', sizeChange);
        } else {
          $visContainer.text('There is no graph to render.').addClass(
            'alert-error');
        }
  ...
  function handleSelection(data, show_result_only) {
    ...
    var hasSelectedRels = data.links.filter(function(link) {
      return link.selected;
    }).length > 0
    for (i = 0; i < data.links.length; i++) {
      var link = data.links[i];
      if (link.selected || (!hasSelectedRels && data.nodes[link.source].selected &&
  ...
  function renderTables() {
    findPreviousQueryWrapper('h5.result-table', $content, function($heading,
      $wrapper) {
      ...
      if (!renderTable($tableContainer, $wrapper.data('data'))) {
        $tableContainer.text("Couldn't render the result table.").addClass(
          'alert-error');
      }
    });
  }
  ...
  function toggler($target, button, action) {
    ...
    if ((action && action === 'hide') || (action === undefined &&
        stateIsExpanded)) {
      $target.hide();
  ...
  function findQuery(selector, context, operation) {
    $(selector, context).each(
      function() {
        $(this).nextAll('div.listingblock').children('div').children(
            'pre.highlight')
  ...
  function errorMessage(message, gist) {
    var messageText;
    if (gist) {
      messageText = 'Something went wrong fetching the GraphGist "' + gist +
        '":<p>' + message + '</p>';
    } else {
      messageText = '<p>' + message + '</p>';
    }

    $content.html('<div class="alert alert-block alert-error"><h4>Error</h4>' +
      messageText + '</div>');
  }
}
